...
use better_subscriber::{layer, prelude::*, reload::*};
...
fn reload_handle() {
    ...
    impl<S: Subscriber> better_subscriber::Layer<S> for Filter {
...
fn reload_filter() {
    struct NopLayer;
    impl<S: Subscriber> better_subscriber::Layer<S> for NopLayer {
    ...
    impl<S: Subscriber> better_subscriber::layer::Filter<S> for Filter {
        fn enabled(&self, m: &Metadata<'_>, _: &layer::Context<'_, S>) -> bool {
            println!("ENABLED: {:?}", m);
            match self {
                Filter::One => FILTER1_CALLS.fetch_add(1, Ordering::SeqCst),
    ...
    let dispatcher = tracing_core::Dispatch::new(
        better_subscriber::registry().with(NopLayer.with_filter(filter)),
    );
