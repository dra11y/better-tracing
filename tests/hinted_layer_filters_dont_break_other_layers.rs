...
use better_subscriber::{filter::DynFilterFn, layer::Context, prelude::*};
...
fn mixed_layered() {
    ...
    let subscriber = better_subscriber::registry().with(layered1).with(layered2);
    assert_eq!(subscriber.max_level_hint(), None);
    let _subscriber = subscriber.set_default();
...
fn filter<S>() -> DynFilterFn<S> {
    DynFilterFn::new(
        (|metadata: &Metadata<'_>, _: &better_subscriber::layer::Context<'_, S>| {
