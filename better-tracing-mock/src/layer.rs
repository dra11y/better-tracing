...
//! ```
//!
//! [`Layer`]: trait@better_subscriber::layer::Layer
use std::{
...
/// let _subscriber =  better_subscriber::registry()
///     .with(
///         layer_2.with_filter(better_subscriber::filter::filter_fn(move |_meta| true))
///     )
///     .set_default();
/// {
///     let _subscriber =  better_subscriber::registry()
///         .with(
///             layer_1
///                 .with_filter(better_subscriber::filter::filter_fn(move |_meta| true))
///         )
///         .set_default();
///
///     tracing::info!("a");
...
impl MockLayerBuilder {
    ...
    ///
    /// ```should_panic
    /// use better_tracing_mock::{layer, expect};
    /// use better_subscriber::{layer::SubscriberExt, util::SubscriberInitExt, Layer};
    ///
    ...
    ///         MockEvent,
    ///     ),
    /// ]', better-tracing-mock/src/subscriber.rs:472:13
    /// ```
    ...
    /// # Examples
    ///
    /// The following test is used within the `better-subscriber`
    /// codebase:
    ///
    /// ```
    /// use tracing::Subscriber;
    /// use better_tracing_mock::layer;
    /// use better_subscriber::{layer::SubscriberExt, util::SubscriberInitExt, Layer};
    ///
...
impl MockLayer {
    fn check_event_scope<C>(
        &self,
        current_scope: Option<better_subscriber::registry::Scope<'_, C>>,
        expected_scope: &mut [ExpectedSpan],
    ) where
        C: for<'lookup> better_subscriber::registry::LookupSpan<'lookup>,
    {
